...
import { getMaterials, getMaterialsMeta, updateMaterialStatus } from '../api/api';
...
export default function Materials() {
  ...
  const [meta, setMeta] = useState({ sectors: DEFAULT_SECTORS, cpses: [], statuses: [] });
  ...
  const load = useCallback(async (searchQuery, targetSector, targetStatus, targetOffset) => {
    setLoading(true);
    try {
      const params = {
        q: searchQuery,
  ...
  useEffect(() => {
    load(debouncedQ, sector, status, offset);
  }, [debouncedQ, sector, status, offset, load]);
  ...
  const sectorsList = meta.sectors.length ? meta.sectors : DEFAULT_SECTORS;

  return (
    <div>
      <div className="page-header">
        <h1>Materials Browser</h1>
        <p>Search, browse, and govern all materials across CPSEs · <strong>{formatNumber(total)}</strong> total records</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="input-row" style={{ flexWrap: 'wrap', gap: 10 }}>
          ...
          <select className="input" style={{ width: 190 }} value={sector} onChange={handleSector}>
            <option value="">All Sectors ({sectorsList.length})</option>
            {sectorsList.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <select className="input" style={{ width: 150 }} value={status} onChange={handleStatus}>
          ...
          <button className="btn btn-primary btn-sm" onClick={() => load(debouncedQ, sector, status, offset)}>
            Refresh
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 380px' : '1fr', gap: 16 }}>
        <div className="card" style={{ padding: 0 }}>
          {loading ? (
            ...
            <EmptyState title="No materials found" desc="Adjust your search or filter criteria." />
          ) : (
            <>
                  ...
                  <tbody>
                    {items.map(m => (
                      <tr key={m.id} style={{ cursor: 'pointer', background: selected?.id === m.id ? 'var(--primary-light)' : '' }} onClick={() => setSelected(m)}>
                        ...
                        <td className="font-mono text-sm" style={{ color: 'var(--primary)', fontWeight: 600 }}>{m.cnmc_code}</td>
              ...
              <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--border)' }}>
                <span className="text-sm text-muted">Page {currentPage} of {pages} · {formatNumber(total)} records</span>
        ...
        {selected && (
          <div className="card" style={{ position: 'sticky', top: 20, alignSelf: 'flex-start' }}>
            ...
            {[
              ...
              { label: 'Unit Price', val: selected.unit_price > 0 ? formatINR(selected.unit_price) : '—' },
              { label: 'Stock Qty', val: formatNumber(selected.stock_qty) },
              { label: 'Annual Qty', val: formatNumber(selected.annual_qty) },
              ...
              <>
                ...
                {Object.entries(selected.extra_attributes).slice(0, 8).map(([k, v]) => (
                  <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, padding: '3px 0' }}>
                    ...
                    <span style={{ maxWidth: 160, textAlign: 'right' }} className="truncate">{String(v)}</span>
                  </div>
                ))}
              </>
            )}
